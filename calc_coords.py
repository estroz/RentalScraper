...
def mapSectors(sector_list, lat, lon, zoom):
    ...
    mx, my = conv.LatLonToMeters(lat, lon)
    px, py = conv.MetersToPixels(mx, my, zoom)
    ...
    #   View the grid by pasting contents of sector_list.txt into input box at: http://www.darrinward.com/lat-long/
    #   width_diff is used to approximate differences in overlap between calculated positions, as maps are displayed centered instead of from the top left corner (which crops part of the map off, decreasing listings)
    for i in range(3, 13):
        ...
        for j in range(0, 8):
            ...
            if spurious_flag != 1:
                ...
                p2mx, p2my = conv.PixelsToMeters(center_x, center_y, zoom)
                m2lx, m2ly = conv.MetersToLatLon(p2mx, p2my)
                sector_pos = {
                    "latitude":     m2lx,
                    "longitude":    m2ly
                }
